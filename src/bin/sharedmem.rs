...
struct Args {
    ...
    // Shared memory key
    // let shmem_key = "my_shared_counter";

    // Attempt to create a new shared memory segment
    //let shmem = ShmemConf::new()
    //    .size(std::mem::size_of::<AtomicCell<SharedQueueMutex>>())
    //    .os_id(shmem_key)
    //    .create()
    //    .unwrap_or_else(|_| {
    ...
    //            .os_id(shmem_key)
    //            .open()
    //            .expect("Failed to open existing shared memory segment.")
    //    });
...
fn increment_value(shmem_flink: &str, thread_num: usize) {
    ...
    let shmem = match ShmemConf::new().size(8192).flink(shmem_flink).create() {
        Ok(m) => m,
    ...
    loop {
        ...
        std::thread::sleep(std::time::Duration::from_millis( rand::random::<u64>() % 1000) );
    }
}
